...
import AppBrand from '../apps/AppBrand'
...
function Header(): JSX.Element {
  ...
  // Logged-out chrome (homepage / pricing / login / intent): minimal — JetCore
  // brand (red) + window controls only. No command bar / dock toggle / focus.
  if (view === 'marketing' || view === 'pricing' || view === 'login' || view === 'intent') {
    return (
      <header className="header drag" style={{ display: 'flex', alignItems: 'center' }}>
        <div className="brand no-drag">
          <Logo size={28} tint="#ff3b3b" />
          <span className="bname">JetCore</span>
        </div>
        <div style={{ flex: 1 }} />
  ...
  return (
    <header className="header drag">
      {/* Static current-app brand (switching lives in the left app rail). */}
      <AppBrand />
