    ...
    return (item: CommandItem): void => {
      switch (item.kind) {
        ...
        case 'pinned-site': {
          ...
          // Use the TARGET workspace's actual partition so the new panel joins
          // that deck's existing (account-scoped) session, not a freshly-derived
          // one. Fall back to the account-scoped scheme if it has none stored.
          const target = useStore.getState().workspaces.find((w) => w.id === workspaceId)
          const partition =
            target?.partition ?? useStore.getState().makePartition(workspaceId)
