...
/** Every storage key currently on disk (encrypted blobs). Used for migration. */
export function listTokenKeys(): string[] {
  return Object.keys(readAll())
}

/**
 * Read the raw (still-encrypted) base64 blob at `key`, or null if absent. This is
 * for COPYING an encrypted value to another key (e.g. account-scoping migration)
 * without ever decrypting it. Do not log the result.
 */
export function getRawBlob(key: string): string | null {
  return readAll()[key] ?? null
}

/**
 * Write a raw (already-encrypted) base64 blob at `key`. Used only to copy an
 * existing encrypted value to a new key during migration; callers must pass a
 * blob obtained from getRawBlob (never plaintext).
 */
export function setRawBlob(key: string, blob: string): void {
  const data = readAll()
  data[key] = blob
  writeAll(data)
}
