...
export default function AILogPanel({ portfolioId, isAiControlled, user }) {
  ...
  function handleRetrain() {
    ...
    api.post('/ai/model/retrain')
      .then(r => {
        const d = r.data || {}
        setRetrainMsg(d.ok ? `✓ ${d.real_samples || 0} real samples` : 'failed')
      })
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <button
              ...
              disabled={retraining}
              style={{
                marginLeft: 'auto', padding: '2px 8px', border: '1px solid rgba(179,157,255,0.35)',
                borderRadius: 4, cursor: retraining ? 'default' : 'pointer', fontSize: 9, fontWeight: 700,
                background: 'rgba(179,157,255,0.10)', color: '#b39dff', fontFamily: 'var(--font-sans)',
                opacity: retraining ? 0.6 : 1, whiteSpace: 'nowrap',
              }}
              title="Retrain the trading model on the latest closed-trade outcomes"
            >
              {retraining ? 'Retraining…' : (retrainMsg || 'Retrain model')}
            </button>
            {user?.username === 'thatonejet' && (
              <button
                onClick={() => setClearModal(true)}
                style={{
                  marginLeft: 6, padding: '2px 8px', border: '1px solid rgba(255,71,111,0.3)',
