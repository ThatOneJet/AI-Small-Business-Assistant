...
function AIIntelTab({ detail, aiLoading, aiError }) {
  ...
  const { market_state, uncertainty, summary, what_changed, breakdown, score, weights_used, reasoning } = detail
  ...
  return (
    <div className="ap-content">
      ...
      {/* Model Thought Process — each live signal's marginal push on the score */}
      <div className="ap-card">
        <div className="ap-card-hd">
          <div className="ap-dot" style={{ background: '#b39dff' }} />
          Model Thought Process
        </div>
        <div style={{ fontSize: 8.5, color: 'var(--t-4)', marginBottom: 9, lineHeight: 1.5, letterSpacing: '0.02em' }}>
          How each signal pushed the model's score, biggest driver first.
        </div>
        {Array.isArray(reasoning) && reasoning.length > 0 ? (
          reasoning.map((r, i) => (
            <ContribBar
              key={i}
              label={`${r.signal} = ${r.value}`}
              contrib={r.contribution ?? 0}
            />
          ))
        ) : (
          <div style={{ fontSize: 9.5, color: 'var(--t-4)', lineHeight: 1.5 }}>
            No decisive signals — the model sees no clear edge here.
          </div>
        )}
      </div>
