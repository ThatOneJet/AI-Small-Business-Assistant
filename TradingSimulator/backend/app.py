...
import trading_model   # our retrainable trading model — imported at top so sklearn loads
                       # in the main thread (avoids eventlet green-thread import deadlocks)
...
def _init_db():
    with _get_db() as conn:
        ...
        try:
            ...
            pass
        try:
            conn.execute("ALTER TABLE sim_positions ADD COLUMN entry_features TEXT")  # for model self-training
        except Exception:
            pass
        ...
        # Safe migration: strategy_type on portfolios (AI trading style; read by /api/account)
        try:
            conn.execute("ALTER TABLE portfolios ADD COLUMN strategy_type TEXT NOT NULL DEFAULT 'balanced'")
        except Exception:
            pass
...
def _sim_buy(symbol: str, qty: float, price: float, portfolio_id: int = 1, record: bool = True):
    ...
    with _get_db() as conn:
        ...
        if existing:
            ...
            _ef = _entry_features.get((portfolio_id, symbol))
            conn.execute(
                "INSERT INTO sim_positions (symbol, shares, avg_cost, realized_pl, portfolio_id, created_at, entry_features) VALUES (?,?,?,0,?,datetime('now'),?)",
                (symbol, qty, price, portfolio_id, _json.dumps(_ef) if _ef else None)
            )
...
def _sim_sell(symbol: str, qty: float, price: float, portfolio_id: int = 1, record: bool = True) -> float:
    with _get_db() as conn:
        pos = conn.execute(
            'SELECT shares, avg_cost, entry_features FROM sim_positions WHERE symbol = ? AND portfolio_id = ?',
            (symbol, portfolio_id)
        ).fetchone()
        ...
        if new_shares < 0.0001:
            _log_trade_outcome(pos['entry_features'], True, pos['avg_cost'], price)  # learn from this long
            conn.execute(
...
def _sim_short(symbol: str, qty: float, price: float, portfolio_id: int = 1, record: bool = True):
    ...
    with _get_db() as conn:
        ...
        if existing:
            ...
            _ef = _entry_features.get((portfolio_id, symbol))
            conn.execute(
                'INSERT INTO sim_positions (symbol, shares, avg_cost, realized_pl, portfolio_id, entry_features) VALUES (?,?,?,0,?,?)',
                (symbol, -qty, price, portfolio_id, _json.dumps(_ef) if _ef else None)
            )
...
def _sim_cover(symbol: str, qty: float, price: float, portfolio_id: int = 1, record: bool = True) -> float:
    ...
    with _get_db() as conn:
        ...
        if abs(remaining) < 0.0001:
            _log_trade_outcome(pos['entry_features'], False, pos['avg_cost'], price)  # learn from this short
        ...
        for sym in batch:
            try:
                ...
                entry = {
                    ...
                    'summary': detail.get('summary', '')[:240],
                    'reasoning': detail.get('reasoning', []),   # per-signal thought process
...
# Any LocalAIDownError that escapes a route -> a clean 503 with the custom message,
# so the frontend can detect the AI being offline no matter which endpoint was hit.
try:
    from llm_trader import LocalAIDownError as _LocalAIDownError

    @app.errorhandler(_LocalAIDownError)
    def _handle_local_ai_down(e):
        return jsonify({
            'error': 'Local AI is offline',
            'detail': str(e) or 'The local AI (Ollama) is not reachable, so the AI '
...
# ── Performance memory: what the AI's own past decisions actually earned ──────────
# Summarized from realized trades + PerformanceEngine and fed into the LLM prompt so
# it learns from its track record (favour winners, avoid losers) — no weight training.
_PERF_MEM_CACHE = {}   # pid -> (ts, memory_str)
_scan_ctx = threading.local()   # holds the portfolio id for the in-flight scan
_entry_features = {}            # (pid, symbol) -> feature vector at entry, for self-training


def _log_trade_outcome(entry_features_json, is_long, avg_cost, exit_price):
    """When an AI position closes, turn its entry features + realized return into a
    training sample so trading_model learns what actually worked vs. didn't."""
    try:
        if not entry_features_json or not avg_cost:
            return
        import trading_model
        feats = _json.loads(entry_features_json)
        ret = (exit_price - avg_cost) / avg_cost       # long % return
        target = max(-10.0, min(10.0, ret * 200.0))    # +5% -> +10 (capped)
        if not is_long:
            target = -target                           # a profitable short => bearish was correct
        trading_model.add_training_row(feats, target)
    except Exception:
        pass

def _ai_perf_memory(pid: int, ttl: int = 60) -> str:
    if not pid:
        return ''
    now = _time.time()
    c = _PERF_MEM_CACHE.get(pid)
    ...
    try:
        from performance_engine import PerformanceEngine
        pe = PerformanceEngine(DB_PATH)
        by_regime = pe.by_regime(pid, days=60) or {}
        total = by_regime.get('_total', {}) if isinstance(by_regime, dict) else {}
        if isinstance(total, dict) and total.get('trades'):
            lines.append(f"Overall: {total['trades']} closed trades, "
                         f"{total.get('win_rate', 0) * 100:.0f}% win, "
                         f"avg P/L {total.get('avg_pl', 0):+.1f}.")
        good, bad = [], []
        for key, s in by_regime.items():
            if key == '_total' or not isinstance(s, dict):
                continue
            n, wr = s.get('trades', 0), s.get('win_rate', 0.5)
            if n >= 3:
                tag = f"{key}: {n}t {wr * 100:.0f}%win ({s.get('avg_pl', 0):+.1f})"
                (good if wr >= 0.55 else bad).append(tag)
        if good:
            lines.append("Worked (favour): " + "; ".join(good[:6]))
        if bad:
            lines.append("Lost money (avoid/size-down): " + "; ".join(bad[:6]))
        try:
            _conn = sqlite3.connect(DB_PATH)
            _conn.row_factory = sqlite3.Row
            _cut = (datetime.utcnow() - timedelta(days=60)).isoformat()
            _rows = _conn.execute(
                '''SELECT symbol, COUNT(*) n,
                          AVG(CASE WHEN realized_pl>0 THEN 1.0 ELSE 0.0 END) wr,
                          AVG(realized_pl) avg_pl
                   FROM sim_trades
                   WHERE portfolio_id=? AND side IN ('sell','cover')
                     AND status='filled' AND created_at>?
                   GROUP BY symbol HAVING COUNT(*)>=2''', (pid, _cut)).fetchall()
            _conn.close()
            bits = [f"{r['symbol']} {r['n']}t {r['wr'] * 100:.0f}%({r['avg_pl']:+.1f})" for r in _rows]
            if bits:
                lines.append("By symbol: " + "; ".join(bits[:8]))
        except Exception:
            pass
    except Exception:
        pass
    mem = "\n".join(lines) or "No closed trades yet — no track record to learn from."
    _PERF_MEM_CACHE[pid] = (now, mem)
    return mem


def _ai_score_detailed(data: dict) -> dict:
    """Trade analysis via OUR OWN retrainable model (trading_model.py) -- replaces the
    pattern-based scorer. The model is pre-trained on sound TA logic and RE-trains on
    the simulator's own closed-trade outcomes. Always available (local, no GPU/Ollama).
    (The legacy pattern-based code below is retained but bypassed by the early return.)
    """
    import trading_model
    ...
    data['_features'] = trading_model.featurize(data)    # remember features to learn from the outcome
    _pid_ctx = getattr(_scan_ctx, 'pid', None)
    if _pid_ctx and _symbol:
        _entry_features[(_pid_ctx, _symbol)] = data['_features']
    if abs(_score) >= 5.0:
        _q = "Strong " + ("BUY" if _score > 0 else "SELL") + " signal"
    elif abs(_score) >= 2.5:
        _q = "Moderate " + ("buy" if _score > 0 else "sell")
    else:
        _q = "No clear edge"

    # ── The model's thought process: which signals drove this score ──────────────
    _top = _reasons[:4]
    _driver_txt = "; ".join(
        f"{r['signal']} {r['value']} ({r['contribution']:+.1f})" for r in _top)
    _summary = (f"{_q} — model score {_score:+.1f}. Drivers: {_driver_txt}"
                if _top else f"{_q} — model score {_score:+.1f} (no strong signals)")
    _breakdown = {r['signal']: r['contribution'] for r in _reasons}

    # what-changed tracking (kept from the original, on the new AI score)
    _changes = []
    _prev = _prev_signals.get(_symbol, {})
    if _prev.get('market_state') and _prev['market_state'] != _market_state:
        _changes.append(f"Market state: {_prev['market_state']} → {_market_state}")
    if _symbol:
        _prev_signals[_symbol] = {'market_state': _market_state, 'score': _score}

    return {
        'score':        _score,
        'market_state': _market_state,
        'breakdown':    _breakdown,
        'reasoning':    _reasons,        # full per-signal thought process
        'uncertainty':  0.0,
        'summary':      _summary,
    ...
    # ── legacy pattern-based analysis (now bypassed by the return above) ──────────
    rsi      = float(data.get('rsi', 50) or 50)
    macd_x   = data.get('macd_cross', '') or ''
...
def _ai_run_portfolio(pid: int) -> dict:
    ...
    Gated on the local AI: if Ollama is offline, the cycle makes NO calls and NO
        ...
        for sym in batch:
            try:
                ...
                batch_data.append({
                    ...
                    'summary': detail.get('summary', '')[:240],       # model's read, shown in scan history
                    'reasoning': detail.get('reasoning', []),          # per-signal thought process
                })
                if qualifies_long:
